...
import { switchUnpaidOrderToCash } from "@/lib/connect-service";
import { corsPreflight, withCors } from "@/lib/cors";
import { checkRateLimit, ORDER_IP } from "@/lib/rate-limit";
import { verifyReceiptToken } from "@/lib/receipt-token";

/**
 * POST /api/v1/orders/{orderId}/pay-cash — the guest switches their unpaid
 * online order to "pay cash at the restaurant", which sends it to the kitchen.
 *
 ...
 * in `switchUnpaidOrderToCash`; this route only maps its answer to HTTP.
 */
...
export async function POST(
  ...
  const result = await switchUnpaidOrderToCash(verified.tenantId, orderId, parsed.data.token);
  if (!result.ok) {
    const status =
